    ...
    return {
        ...
        controller:function($scope, $element, $compile, $templateRequest){
            ...
            var updatePopup = function (coordinate) {
                ...
                console.debug($scope.features[$scope.selected].olFeature)
            ...
            map.on('singleclick', function(evt) {
                ...
                angular.forEach($scope.map.overlays, function (layer, index) {
                    ...
                    // var extent = new ol.geom.Point(evt.coordinate).getExtent();
                    // var poly = ol.geom.Polygon.fromExtent(extent);
                    // poly = ol.proj.transform(poly, 'EPSG:3857','EPSG:4326');
                    // console.debug(new ol.format.WKT().writeGeometry(poly));
                    // var params = {
                    //     service: 'WFS',
                    //     version: '2.0.0',
                    //     request: 'GetFeature',
                    //     typename: source.getParams().LAYERS,
                    //     outputFormat: 'application/json',
                    //     srsname: 'EPSG:3857',
                    //     CQL_FILTER: "INTERSECTS(the_geom,"+new ol.format.WKT().writeGeometry(poly)+")"
                    // };
                    ...
                    $http.get(url).then(function(response) {
                        ...
                        angular.forEach(features, function(f, index){
                            ...
                            feature.properties = [];
                            feature.olFeature = f;
                            console.debug(f.getGeometry().clone())
