...
from .models import Server, SimpleAuthConnection, TokenAuthConnection, Connection


class ServerAdminInline(admin.TabularInline):
    model = Server
...
class ServerAdmin(admin.ModelAdmin):
    ...
    def harvest_resources(self, request, queryset):
        for server in queryset:
            harvest_task.delay(server_id=server.id, user_id=request.user.id)
        self.message_user(
    ...
    def update_resources(self, request, queryset):
        for server in queryset:
            update_server_resources.delay(server_id=server.id, user_id=request.user.id or None)
        self.message_user(
...
class ConnectionAdmin(guardian_admin.GuardedModelAdmin):
    list_display = ('title', 'auth_type')


@admin.register(SimpleAuthConnection)
class SimpleAuthConnectionAdmin(guardian_admin.GuardedModelAdmin):
    list_display = ('title', 'username', 'auth_type')
...
class TokenAuthConnectionAdmin(guardian_admin.GuardedModelAdmin):
    list_display = ('title', 'prefix', 'token')
